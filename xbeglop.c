...
{
    if(argc != 3) {
        printf("Usage: lops beginning off a wav file. Args: 1) Name of wavfile 2) mm:ss.hh string say to where wav must be deleted.\n");
        exit(EXIT_FAILURE);
    }
    /* parse edit file in very simple terms, that means using scanf */
    FILE *inwavfp;
    inwavfp = fopen(argv[1],"rwb");
    if ( inwavfp == NULL ) {
    ...
    fwrite(inhdr, sizeof(char), 44, inwavfp);

    bf=realloc(bf, inhdr->byid);
