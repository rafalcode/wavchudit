...
{
    if(argc == 1) {
        printf("Usage: \"./sanwav file.wav -f\", omit the -f (aka. force) is you do not want the byid field to be overwritten by size info)\n");
        exit(EXIT_FAILURE);
    }

    unsigned char ovwbyid=0;
    if(argc==3) {
        if((argv[2][0]=="-") & (argv[2][1]=="f") )
            ovwbyid=1;
        else {
            printf("Only the -f option (force overwrite) is allowed as second argument.\n");
            exit(EXIT_FAILURE);
        }
    }
    ...
    struct stat fsta;
    FILE *wavfp;
    unsigned whsz=malloc(sizeof(wh_t)); // of course will always be 44
    wh_t *inhdr=malloc(whsz);
    int ret;

    if (stat(argv[1], &fsta) == -1) { /*  remember there is also lstat: it does not follow symlink */
        printf("Could not stat given file.\n"); 
        fprintf(stderr, "%s\n", strerror(errno));
    } else {
        wavfp = fopen(argv[1],"r+b");
        if ( wavfp == NULL ) {
